...
# are still missing AWM completion reporting.
...
reason="Stop blocked: this session has file edits that have not been closed with awm done."
if [ ! -f "$WORK_MARKER" ] && [ -f "$FILES_TRACKER" ]; then
  line_count=$(grep -c '.' "$FILES_TRACKER" || true)
  if [ "${line_count}" -gt 1 ]; then
    reason="${reason} Create or update /awm-work before continuing multi-file work."
  fi
fi
reason="${reason} Run /awm-verify for executable, config, contract, onboarding, or behavior changes, then finish with /awm-done before ending the task."
