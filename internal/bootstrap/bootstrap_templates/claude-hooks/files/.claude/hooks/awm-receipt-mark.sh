#!/bin/bash
# PostToolUse hook: detect AWM task-loop commands and update session
...
# `awm context`, `awm work`, `awm verify`, and `awm done`
# invocations across direct CLI, `awm run --in <request.json>`, and
# `awm-mcp invoke --tool ...` forms.
...
is_task_context_command() {
  local command="$1"
  echo "$command" | grep -qE '(^|[[:space:]])awm[[:space:]]+context([[:space:]]|$)' || return 1
...
is_direct_work_command() {
  local command="$1"
  echo "$command" | grep -qE '(^|[[:space:]])awm[[:space:]]+work([[:space:]]|$)' || return 1
  ! echo "$command" | grep -qE '(^|[[:space:]])awm[[:space:]]+work[[:space:]]+(list|search)([[:space:]]|$)'
}

is_direct_verify_command() {
  local command="$1"
  echo "$command" | grep -qE '(^|[[:space:]])awm[[:space:]]+verify([[:space:]]|$)'
}

is_direct_done_command() {
  local command="$1"
  echo "$command" | grep -qE '(^|[[:space:]])awm[[:space:]]+done([[:space:]]|$)'
}
...
is_mcp_tool_command() {
  local command="$1"
  local tool_name="$2"
  echo "$command" | grep -qE '(^|[[:space:]])awm-mcp[[:space:]]+invoke([[:space:]]|$)' || return 1
...
elif echo "$COMMAND" | grep -qE '(^|[[:space:]])awm[[:space:]]+run([[:space:]]|$)'; then
  INPUT_PATH=$(extract_awm_input_path "$COMMAND")
