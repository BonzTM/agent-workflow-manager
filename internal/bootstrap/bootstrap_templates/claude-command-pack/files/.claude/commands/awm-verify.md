...
4. If a third argument is present, treat it as `--phase`; otherwise let AWM default the phase.
5. Run `awm verify` with the parsed identifiers and any explicit changed files.
...
- When receipt baseline detection is unavailable and changed-path selectors matter, pass files explicitly instead of assuming AWM can infer them.
- If selection is unexpectedly empty, inspect the repo’s `.awm/awm-tests.yaml` selectors before forcing completion.
