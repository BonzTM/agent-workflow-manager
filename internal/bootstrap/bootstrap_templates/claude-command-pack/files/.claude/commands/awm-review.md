...
3. If the JSON contains `run=true`, run `awm review --run` with the parsed identifier and any optional `tags_file`.
4. Otherwise map any supplied `key`, `summary`, `status`, `outcome`, `blocked_reason`, and `evidence` values onto the direct `awm review` flags and run the command.
