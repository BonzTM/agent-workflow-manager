...
2. Run `awm context --phase <phase> --task-text "<task_text>"`.
...
5. If a returned plan, task, or pointer key actually needs to be hydrated, run `awm fetch --receipt-id <receipt_id>` first and add explicit `--key` flags only when you need a narrower fetch.
6. Return a structured summary with `receipt_id`, hard rules, active plans, any known initial scope, and fetched artifacts.

Constraints:
- Do not skip the `awm context` call.
- Do not downgrade hard rules into optional guidance.
- Do not fabricate file scope. Use `/awm-work` to declare discovered paths when governed work expands beyond the initial receipt scope.
