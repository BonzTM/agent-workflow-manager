...
`work` can also auto-open a receipt: pass `--task-text` (optionally `--phase`) with no `--plan-key`/`--receipt-id` and a single call starts durable governed state; explicit identifiers always win, and repos relying on strict scope checks should still open receipts with `context` before editing.
